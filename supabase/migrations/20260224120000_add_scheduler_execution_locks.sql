...
BEGIN
  DELETE FROM scheduler_execution_locks AS sel
  WHERE sel.locked_until <= now();
  ...
  WITH lock_attempt AS (
    ...
    RETURNING
      true AS acquired,
      scheduler_execution_locks.lock_token,
      scheduler_execution_locks.locked_until
  )
  SELECT la.acquired, la.lock_token, la.locked_until
