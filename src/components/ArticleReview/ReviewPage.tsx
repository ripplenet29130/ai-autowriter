  ...
  return <main className="min-h-screen bg-gray-100 text-gray-900">
    ...
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-6 p-4 lg:p-8">
      <article className="bg-white border border-gray-200 rounded-xl p-6 lg:p-10 shadow-sm">
        ...
        {canComment && !canEdit && selection && <div className="fixed inset-x-0 bottom-0 z-30 border-t border-blue-200 bg-white/95 p-4 shadow-[0_-8px_24px_rgba(15,23,42,0.12)] backdrop-blur"><div className="mx-auto max-w-3xl rounded-xl border border-blue-200 bg-blue-50 p-4"><div className="flex items-start justify-between gap-4"><p className="text-sm text-blue-900">「{selection.text.length > 80 ? `${selection.text.slice(0, 80)}…` : selection.text}」へのコメント</p><button onClick={() => { setSelection(null); setCommentBody(''); window.getSelection()?.removeAllRanges(); }} className="shrink-0 rounded p-1 text-blue-700 hover:bg-blue-100" title="コメント入力を閉じる"><X className="w-4 h-4" /></button></div><div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end"><textarea autoFocus value={commentBody} onChange={e => setCommentBody(e.target.value)} placeholder="コメントを入力" rows={2} className="input-field flex-1" /><button onClick={addComment} disabled={saving || !commentBody.trim() || !authorName.trim()} className="btn-primary shrink-0 flex items-center justify-center gap-2 disabled:opacity-50"><Send className="w-4 h-4" />コメントを投稿</button></div></div></div>}
      </article>
