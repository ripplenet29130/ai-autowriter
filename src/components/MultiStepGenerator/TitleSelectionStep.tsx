...
import { Type, Sparkles, ArrowRight, Check, Info, ArrowLeft } from 'lucide-react';
  ...
  if (isLoading && (!displayTitles || displayTitles.length === 0)) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-lg font-medium text-gray-900">タイトル候補を生成中...</p>
        <p className="text-sm text-gray-500 mt-2">分析結果をもとに最適なタイトルを作成しています</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-900 flex items-center space-x-2">
            ...
            <span>{titleSet ? 'タイトルの選択' : 'タイトル候補の選択'}</span>
          </h3>
          <p className="text-gray-600 mt-1">
            {titleSet
              ? `「${titleSet.name}」から使用するタイトルを選択してください`
              : 'AIが作成した候補タイトルから、最適なものを選択してください'}
          </p>
        </div>
        {!titleSet && (
          <button
            ...
            <div className="flex items-start justify-between">
              ...
              <div
                className={`
                  ...
                  ${selectedId === suggestion.id ? 'bg-blue-500 border-blue-500 text-white' : 'border-gray-300'}
                `}
              >
      ...
      <div className="flex justify-between items-center pt-6 border-t border-gray-100">
        <button onClick={onBack} className="btn-secondary flex items-center space-x-2">
          ...
          <span>{titleSet ? '設定へ戻る' : '分析結果へ戻る'}</span>
        </button>
        <div className="flex items-center text-sm text-gray-500">
          <Info className="w-4 h-4 mr-1.5" />
          <span>タイトルを選んで次へ進んでください</span>
        </div>
        <button onClick={onNext} disabled={!selectedId} className="btn-primary flex items-center space-x-2">
          <span>アウトライン作成へ</span>
