...
export function buildSchedulerStructureRules(targetWordCount: number): string {
  ...
  if (targetWordCount <= 2500) {
    return [
      '構成ルール:',
      '- Lead 1つ、H2を4〜6つ（最後はまとめ）',
      '- H2の数は扱うべきトピックの数に合わせること。無理に増やさない',
      '- 長くなりそうなH2には、直後にH3を1〜2つ置いて論点を分ける',
      '- 記事全体でH3を2〜4つ追加。ただし全H2に機械的に付けない',
  ...
  return [
    '構成ルール:',
    '- Lead 1つ、H2を5〜7つ（最後はまとめ）',
    '- H2の数は扱うべきトピックの数に合わせること。無理に増やさない',
    '- 長くなりそうなH2には、直後にH3を1〜2つ置いて論点を分ける',
    '- 記事全体でH3を3〜6つ追加。ただし全H2に機械的に付けない',
    '- 各H2は異なるトピックを担当すること。同じテーマを別の表現で繰り返さない',
    '- 網羅性と重複回避を両立する',
