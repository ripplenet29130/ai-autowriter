...
export async function generateTitleSuggestionsWithSharedCore(
  ...
  const outputExample = Array.from({ length: count }, (_, index) =>
    `  { "title": "タイトル${index + 1}", "reason": "狙いを20字以内で記載" }`
  ).join(',\n');

  const prompt = `
あなたは日本語SEOライターです。以下のキーワードで、ブログ記事タイトルを${count}件作成してください。
...
【タイトル作成ルール】
1. 既存記事を模倣しない
2. ${count}件それぞれ異なる切り口にする
3. メインキーワードを自然に含める
4. titleは24〜68字
5. reasonは20字以内
6. 解説、前置き、Markdown、コードブロックは禁止

【出力形式】
JSON配列のみ。次の形以外は出力しない:
[
${outputExample}
]
`.trim();
  ...
  try {
    const raw = await params.callAI(prompt, 3200);
...
`.trim();
    const retryRaw = await params.callAI(retryPrompt, 2400);
