  ...
  return <main className="min-h-screen bg-gray-100 text-gray-900">
    ...
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-6 p-4 lg:p-8">
      <article className="bg-white border border-gray-200 rounded-xl p-6 pb-64 lg:p-10 lg:pb-56 shadow-sm">
        {canEdit && <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="w-full max-w-xs">{viewMode === 'edit' && <><label className="mb-1 block text-xs font-medium text-gray-500">編集者名</label><input value={authorName} onChange={e => setAuthorName(e.target.value)} placeholder="例：山田 花子" className="input-field" /></>}</div><div className="inline-flex rounded-lg bg-gray-100 p-1"><button onClick={() => setViewMode('preview')} className={`rounded-md px-4 py-2 text-sm font-medium transition ${viewMode === 'preview' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-600'}`}>プレビュー</button><button onClick={() => setViewMode('edit')} className={`rounded-md px-4 py-2 text-sm font-medium transition ${viewMode === 'edit' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-600'}`}>編集</button></div></div>}
        {saving && <div className="mb-6 text-sm text-blue-600">保存中…</div>}
        {canEdit && viewMode === 'edit' ? <EditableArticle article={review.article} onSave={saveArticle} saving={saving} /> : <ReadArticle article={review.article} rootRef={articleRef} commentTexts={comments.filter(comment => comment.status === 'open').map(comment => comment.selectedText).filter((text): text is string => Boolean(text))} onSelect={() => { const value = articleRef.current && selectedOffsets(articleRef.current); if (value) setSelection(value); }} />}
        {canComment && viewMode === 'preview' && <div className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-4 py-5 shadow-[0_-10px_30px_rgba(15,23,42,0.14)] backdrop-blur"><div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-3 flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-xs font-semibold tracking-wide text-slate-500">コメントする箇所</p>{!authorName.trim() ? <p className="mt-1 text-sm text-slate-500">レビュー参加者名をご記入いただくとコメントを入力できます</p> : selection ? <p className="mt-1 truncate text-sm font-medium text-slate-800">「{selection.text}」へのコメント</p> : <p className="mt-1 text-sm text-slate-500">本文を選択するとコメントを入力できます</p>}</div><button onClick={() => { setSelection(null); setCommentBody(''); window.getSelection()?.removeAllRanges(); }} className="shrink-0 rounded-md border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"><X className="mr-1 inline w-3.5 h-3.5" />選択箇所をリセット</button></div><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><div className="sm:w-44"><label className="mb-1 block text-xs font-medium text-slate-600">レビュー参加者名</label><input value={authorName} onChange={e => setAuthorName(e.target.value)} onBlur={() => authorName.trim() && localStorage.setItem(`reviewer:${token}`, authorName.trim())} placeholder="例：山田 花子" className="input-field" /></div><textarea value={commentBody} onChange={e => setCommentBody(e.target.value)} placeholder={!authorName.trim() ? '先にレビュー参加者名を入力してください' : selection ? 'コメントを入力' : '本文を選択してからコメントを入力'} rows={2} className="input-field flex-1 resize-none" disabled={!authorName.trim() || !selection} /><button onClick={addComment} disabled={saving || !selection || !commentBody.trim() || !authorName.trim()} className="btn-primary min-w-36 shrink-0 flex items-center justify-center gap-2 disabled:opacity-50"><Send className="w-4 h-4" />コメントを投稿</button></div></div></div>}
      </article>
