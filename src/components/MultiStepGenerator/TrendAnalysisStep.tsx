    ...
    <div className="space-y-6">
          ...
          {trendData.competitorAnalysis.topArticles.map((article, index) => (
            <div key={index} className="bg-white border border-gray-200 rounded-xl hover:shadow-md transition-all divide-y divide-gray-100 overflow-hidden">
              <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span className="text-xs text-gray-500">約 {article.wordCount.toLocaleString()} 文字</span>
                    </div>
                    <h5 className="font-bold text-gray-900 text-lg leading-tight">{article.title}</h5>
                  </div>
                ...
                <p className="text-sm text-gray-600 line-clamp-2">{article.metaDescription || 'メタディスクリプションなし'}</p>
              </div>
              ...
              {expandedIndex === index && (
                <div className="bg-gray-50/50 p-5 animate-in slide-in-from-top-2 duration-300">
                  ...
                  {article.headings && article.headings.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-500 italic py-2">見出し情報を取得できませんでした。</div>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
          ...
          <div className="flex flex-wrap gap-2">
            {trendData.hotTopics.map((topic, index) => (
              <span key={index} className="px-3 py-1 bg-purple-100 border border-purple-300 rounded-full text-sm text-purple-800 font-medium">
                {topic}
              </span>
            ))}
          </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-xs text-gray-500">クリックで切替: 通常 → NG → 重要</span>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {Array.from(new Set([...trendData.relatedKeywords.slice(0, 20), ...Object.keys(keywordPreferences)])).map((keyword, index) => {
            ...
            let baseStyle =
              'px-4 py-2 rounded-full text-sm font-bold transition-all cursor-pointer select-none transform hover:scale-105 active:scale-95 border-2 shadow-sm';
            let statusStyle = '';
            ...
            return (
              <div
                ...
                title={status === 'default' ? 'AIに任せる' : status === 'ng' ? '使わない' : '優先的に使う'}
              >
                {keyword}
                {status === 'essential' && <span className="ml-1.5 text-[10px] bg-white/20 px-1 rounded">重要</span>}
        ...
        <p className="text-[11px] text-gray-400 mt-4 leading-relaxed italic">
          重要は本文で使われやすく、NGは生成対象から除外されます。
        </p>
      </div>

      <div className="flex justify-between items-center pt-8 border-t border-gray-100 mt-8">
        <button onClick={onBack} className="group flex items-center space-x-2 text-gray-500 hover:text-gray-900 font-bold transition-all">
