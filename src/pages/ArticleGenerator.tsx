  ...
  const handlePostToWordPress = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('WordPress投稿エラー:', errorText);
        throw new Error('WordPressへの投稿に失敗しました');
      }

      const postData = await response.json();
      const wpUrl = postData.link || `${wpConfig.url}/?p=${postData.id}`;
      ...
      if (saveError) {
        console.error('記事履歴の保存エラー:', saveError);
      }
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">AI × トレンドキーワードで自動記事生成・WordPress投稿</p>
      </div>
      ...
      {(aiConfigs.length === 0 || wpConfigs.length === 0 || trendKeywords.length === 0) && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            <p className="text-yellow-800 font-medium mb-1">必要な設定が不足しています</p>
        ...
        <div className="space-y-6">
          ...
          {selectedAiConfig && (
            <div className="border-t border-gray-200 pt-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">現在のAI設定</h3>
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p className="font-medium text-gray-800">{selectedAiConfig.article_length}</p>
                </div>
              </div>
            </div>
          )}
      ...
      {generatedArticle && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-8">
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-2xl font-bold text-gray-800 mb-4">{generatedArticle.title}</h3>
      ...
      {!generatedArticle && !generating && (
        <div className="bg-gray-50 rounded-lg p-12 text-center">
          ...
          <p className="text-sm text-gray-500">上のフォームから設定を選択して記事を生成してください</p>
        </div>
      )}
    </div>
  );
