...
import { Check, ChevronLeft, Loader2, MessageSquare, Send } from 'lucide-react';
  ...
  return <main className="min-h-screen bg-gray-100 text-gray-900">
    ...
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-6 p-4 lg:p-8">
      <article className="bg-white border border-gray-200 rounded-xl p-6 lg:p-10 shadow-sm">
        ...
        {canComment && !canEdit && selection && <div className="mt-6 border border-blue-200 bg-blue-50 rounded-lg p-4 space-y-3"><p className="text-sm text-blue-900">「{selection.text.length > 80 ? `${selection.text.slice(0, 80)}…` : selection.text}」へのコメント</p><textarea value={commentBody} onChange={e => setCommentBody(e.target.value)} placeholder="コメントを入力" rows={3} className="input-field" /><button onClick={addComment} disabled={saving || !commentBody.trim() || !authorName.trim()} className="btn-primary flex items-center gap-2 disabled:opacity-50"><Send className="w-4 h-4" />コメントを投稿</button></div>}
      </article>
