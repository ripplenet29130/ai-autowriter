...
function escapeXmlRpcValue(value: string | number): string {
  return String(value)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function isPermissionRelatedWpError(status: number, text: string): boolean {
...
function buildXmlRpcNewPostBody(params: {
  username: string;
  password: string;
  postType: string;
  title: string;
 ...
 */
async function publishViaXmlRpc(
  config: WordPressConfig,
  postType: string,
  title: string,
  content: string,
